...
func StartTestServer(t *testing.T, handlerFunc func(*testing.T) http.HandlerFunc) *httptest.Server {
    soajsMiddleware := soajsGo.InitMiddleware(map[string]string{"serviceName": serviceName})
...
func GetDeployerOpsHandler(t *testing.T) http.HandlerFunc {
    fn := func(w http.ResponseWriter, r *http.Request) {
        log.Println("Deployer operations handler loaded ...")
        ...
        // get deployer
        deployer, err := soajs.Reg.GetDeployer()
        assert.NoError(t, err)
        assert.NotEmpty(t, deployer)
	}
	return http.HandlerFunc(fn)
}
...
func GetDaemonsOpsHandler(t *testing.T) http.HandlerFunc {
    fn := func(w http.ResponseWriter, r *http.Request) {
        log.Println("Daemons operations handler loaded ...")
        ...
        // get all daemons
        daemons, err := soajs.Reg.GetDaemons()
        assert.NoError(t, err)
        assert.NotEmpty(t, daemons)

        // missing daemon name in param
        _, err = soajs.Reg.GetDaemon("")
        assert.Equal(t, errors.New("Daemon name is required"), err)

        // get one daemon
        oneDaemon, err := soajs.Reg.GetDaemon("testDaemon")
        assert.NoError(t, err)
        assert.NotEmpty(t, oneDaemon)
	}
	return http.HandlerFunc(fn)
}
